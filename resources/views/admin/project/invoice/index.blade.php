...
@section('title-page')
    ...
    <span class="text">All Invoices of {{ $project->name }}</span>
@endsection
    ...
    <section class="report-table">
                    ...
                    <div class="widget-block widget-item widget-style">
                        <div class="heading-widget">
                            <div class="row align-items-center">
                                <div class="col-10">
                                    <span class="widget-title">All Invoices of {{ $project->name }}</span>
                                </div>
                                <div class="col-2 left">
                                    <a href="{{ route('invoices.create', $project->id) }}" class="submit-form-btn">New Invoice</a>
                                </div>
                            </div>
                        </div>

                        <div class="widget-content">
                            <form action="{{ route('invoices.destroy') }}" method="post" onsubmit="return confirm('<?php echo "Are you sure you want to delete the selected items?";?>');">
                                @csrf
                                <table class="table align-items-center">
                                    ...
                                    @foreach ($invoices as $item)
                                        <tr>
                                            ...
                                            <td class="center" style="padding-left: 0px;">
                                                <a title="Payments" href="{{ route('projects.invoice', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-plus"><i class="zmdi zmdi-money"></i></a>
                                                <a href="{{ route('invoices.edit', $item->id) }}" class="table-btn table-btn-icon table-btn-icon-edit"><i class="zmdi zmdi-edit"></i></a>
                                            </td>
                                        </tr>
                                    @endforeach
                                    ...
                                    <tr>
                                        <td style="vertical-align: middle;" colspan="8">
                                            <div class="row align-items-center">
                                                <div class="col-4">
                                                    Show items {{ $invoices->firstItem() }} to {{ $invoices->lastItem() }} from {{ $invoices->total() }} item (pages {{ $invoices->currentPage() }} from {{ $invoices->lastPage() }})
                                                </div>
        ...
        @else
            <div class="widget-block widget-item widget-style center no-item">
                ...
                <div class="create-item"><a href="{{ route('invoices.create', $project->id) }}">Add New Invoice</a></div>
            </div>
        @endif

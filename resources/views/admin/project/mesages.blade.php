...
<span class="text">{{ $project->name }} - Messages</span>
@endsection
                ...
                <div class="col-inside-lg decor-default">
                    <div class="chat-body" style="position:relative;">
                        <div class="msg-body">
                            ...
                            <div class="answer left" id="msg-{{ $message->msg_id }}">
                                <div class="avatar">
                                ...
                                <div class="text">
                                    @if($message->file)
                                    <a style="color: rgb(255, 255, 255);" href="{{ 'https://kavax.co.uk/app/files_directory/'.$message->file->file_name }}" target="_blank" download>
                                        <i style="color: #fff;" class="zmdi zmdi-hc-2x zmdi zmdi-download"></i> {{ $message->file->file_name }}</a>
                                    @else
                                    {{ $message->message }}
                                    @endif
                                </div>
                            ...
                            @elseif ($message->replied_by != NULL)
                            <div class="answer right" id="msg-{{$message->msg_id}}">
                                <div class="avatar">
                                    <i style="font-size: 48px;" class="zmdi zmdi-account-circle"></i>
                                </div>
                                <div class="name">{{ $message->replied->fullName }}</div>
                                <div class="text">
                                    @if($message->file)
                                    <a style="color: rgb(255, 255, 255);" href="{{ asset('storage/message/files/'.$message->file->file_name) }}" target="_blank" download>
                                        <i style="color: #fff;" class="zmdi zmdi-hc-2x zmdi zmdi-download"></i> {{ $message->file->file_name }}</a>
                                    @else
                                    {{ $message->message }}
                                    @endif
                        ...
                        <form action="{{ route('project.message.send') }}" method="post" enctype="multipart/form-data" style="position:fixed;bottom:30px;width:75%;">
                            @csrf
                            <input type="hidden" name="project_id" value="{{$project->id}}">
                            <div class="answer-add">
                                <input autocomplete="off" type="text" name="message" placeholder="Write a message" id="msg-box">
...
<script type="text/javascript">
    $(function() {
        $(".chat").niceScroll();
        var viewID = @php echo count($messages) ? $messages->last()->msg_id : 0;
        @endphp


        setTimeout(function() {
            document.getElementById("msg-" + viewID).scrollIntoView({
                behavior: "smooth"
            });
        }, 50);
    ...
    var pusher = new Pusher('9351e3ecb62ec1001121', {
        cluster: 'ap1',
        encrypted: true
    });
    ...
    channel.bind('admin-message', function(data) {
        if (data.message.project_id == @php echo $project->id; @endphp) {



            var msg = `<div class="answer left" id="` + data.message.msg_id + `">
            <div class="avatar">
                <i style="font-size: 48px;" class="zmdi zmdi-account"></i>
            </div>
            <div class="name"> ` + data.message.replied_by + ` </div>
            <div class="text">
            ...
            setTimeout(function() {
                document.getElementById("msg-" + data.message.msg_id).scrollIntoView({
                    behavior: "smooth"
                });
            }, 50);
        }

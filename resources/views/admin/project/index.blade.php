                ...
                <div class="widget-block widget-item widget-style">
                    <div class="heading-widget">
                        <div class="row align-items-center">
                            ...
                            <div class="col-4 left">
                                <form action="{{ url('admin/projects') }}" method="get">
                                    <div class="input-group rounded">
                                        ...
                                        <div>
                                            <a class="btn btn-primary" href="{{ route('newproject') }}"
                    ...
                    @if (count($projects))
                        <div class="widget-content">
                            <form action="{{ route('projects.destroy') }}" method="post"
                                onsubmit="return confirm('<?php echo 'Are you sure you want to delete the selected items?'; ?>');">
                                @csrf
                                <table class="table align-items-center">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Employee</th>
                                            <th>Package</th>
                                            <th width="140px" class="center">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($projects as $item)
                                            <tr>
                                                <td class="delete-col">
                                                    <input class="delete-checkbox" type="checkbox" name="delete_item[]"
                                                        value="{{ $item->id }}">
                                                </td>
                                                <td>{{ $item->name }}</td>
                                                <td>{{ $item->budget }}</td>
                                                ...
                                                <td>{{ $item->end_date }}</td>
                                                <td>{{ $item->status == 1? 'Running': ($item->status == 2? 'Pause': ($item->status == 0? 'New Registered': 'Completed')) }}
                                                </td>
                                                <td>{{ $item->progress }}</td>
                                                <td>{{ $item->employee_size }}</td>
                                                <td>{{ $item->package_name }}</td>
                                                <td class="center" style="padding-left: 0px;">
                                                    <a title="Messages" href="{{ route('project.message', $item->id) }}"
                                                        class="table-btn table-btn-icon table-btn-icon-plus"><i
                                                            class="zmdi zmdi-comment-alert"></i></a>
                                                    <a title="Tasks" href="{{ route('tasks.index', $item->id) }}"
                                                        class="table-btn table-btn-icon table-btn-icon-plus"><i
                                                            class="zmdi zmdi-calendar-check"></i></a>
                                                    @if (Auth::user()->role == 'super_admin')
                                                        <a title="Create Invoice"
                                                            href="{{ route('invoices.index', $item->id) }}"
                                                            class="table-btn table-btn-icon table-btn-icon-plus"><i
                                                                class="zmdi zmdi-money"></i></a>
                                                        <a href="{{ route('projects.edit', $item->id) }}"
                                                            class="table-btn table-btn-icon table-btn-icon-edit"><i
                                    ...
                                    <tfoot>
                                        <tr class="titles">
                                            <th class="delete-col">
                                                <button class="table-btn table-btn-icon table-btn-icon-delete">
                                                    <span><img src="{{ asset('assets/admin/img/base/icons') }}/trash.svg"
                                                            alt="ID" title="Delete"></span>
                                                </button>
                                            </th>
                                            <th>Name</th>
                                            <th>Budget</th>
                                        ...
                                        <tr>
                                            <td style="vertical-align: middle;" colspan="8">
                                                <div class="row align-items-center">
                                                    <div class="col-4">
                                                        Show items {{ $projects->firstItem() }} to
                                                        {{ $projects->lastItem() }} from {{ $projects->total() }} item
                                                        (pages {{ $projects->currentPage() }} from
                                                        {{ $projects->lastPage() }})
                                                    </div>
                    ...
                    @else
                        <div class="widget-block widget-item widget-style center no-item">
                            <div class="icon"><img
                                    src="{{ asset('assets/admin/img/base/icons') }}/no-item.svg"></div>
